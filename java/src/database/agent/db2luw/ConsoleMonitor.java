...
import com.easydataservices.db2admintool.database.agent.ConsoleAgent;
...
//------------------------------------------------------------------------------
// File:         ConsoleMonitor.java
// Licence:      Apache License 2.0
// Description:  
/**
 * Main class for a Db2 for LUW monitoring agent using a console data source.
 * @author jeremy.rickard@easydataservices.com
 * @version 2021.12.04
 */
//------------------------------------------------------------------------------
@Repository
public class ConsoleMonitor extends ConsoleAgent {
  private static final String className = ConsoleMonitor.class.getName();
  ...
  protected void validateProperties(SimpleCommandLinePropertySource propertySource) {
    ...
    final String textUsage = "Usage: " + className 
      + " [ --repositoryFile=<path> ] [ --configFile=<path> ]";
    ...
    if (propertySource.getPropertyNames().length > 2) {
  ...
  protected void exec(AnnotationConfigApplicationContext context) {
    ...
    while (true) {
      ...
      if (pollDelay.getNextPollDelayMilliseconds().equals(0)) {
        ...
        try {
          MonitorSnapshotDao snapshotDao = (MonitorSnapshotDao) Class.forName(job.getExecClass()).newInstance();
...
db2DatabaseIdentity = new Db2DatabaseIdentity();
db2DatabaseIdentity.setDbName("EZSY");
db2DatabaseIdentity.setDbSeed("3065966854");
Integer dbId = 1;
    if (dbId == null) {
  ...
  public static void main(String[] args) {
    ...
    try {
      ConsoleAgent agent = new ConsoleMonitor();
      agent.run(args);
    }
    catch (Exception exception) {
