...
BEGIN
  FOR c AS c1 INSENSITIVE CURSOR WITH HOLD FOR
    SELECT snapshot_timestamp, db_id, agent_id FROM ctl_db2luw.sndbcf WHERE transform_timestamp IS NULL ORDER BY 1, 2, 3 WITH RS
  DO
      ...
      FROM
        ...
        (
          ...
          FROM
            stg_db2luw.sndbcf 
          WHERE
            db_id = c.db_id AND agent_id = c.agent_id AND snapshot_timestamp = c.snapshot_timestamp
        ) AS n
          ON
            t.db_id = n.db_id AND
            t.agent_id = n.agent_id AND
            t.db_member = n.db_member AND
            t.parameter_name = n.parameter_name
      WHERE
        t.db_id = c.db_id AND
        t.agent_id = c.agent_id AND
    ...
    FOR r AS
      ...
      FROM 
        ...
        (
          ...
          FROM
            repo_db2luw.sndbcf AS t
          WHERE
            db_id = c.db_id AND agent_id = c.agent_id AND to_timestamp = '9999-12-31-24.00.00'
        ) AS t
          ON
            n.db_id = t.db_id AND
            n.agent_id = t.agent_id AND
      ...
      WHERE
        n.db_id = c.db_id AND n.agent_id = c.agent_id AND n.snapshot_timestamp = c.snapshot_timestamp
      WITH CS
    DO
      ...
      IF r.is_changed IS NULL THEN
        INSERT INTO repo_db2luw.sndbcf(db_id, agent_id, db_member, parameter_name, from_timestamp, value, value_flags, deferred_value, deferred_value_flags)
        VALUES
          (r.db_id, r.agent_id, r.db_member, r.parameter_name, r.snapshot_timestamp, r.value, r.value_flags, r.deferred_value, r.deferred_value_flags);
      END IF;
    END FOR;

    -- Delete staging data.
    DELETE FROM stg_db2luw.sndbcf 
    WHERE
      db_id = c.db_id AND agent_id = c.agent_id AND snapshot_timestamp = c.snapshot_timestamp;
